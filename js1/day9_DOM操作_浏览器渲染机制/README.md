...
| HTML + CSS + JS | ↓ | DOM 树 / CSSOM 树 |
| --- | --- | --- |
| ↓ | 渲染树 (Render Tree) | ↓ |
| 布局 (Layout / Reflow)  ← 元素大小/位置计算 | ↓ | 绘制 (Paint)             ← 绘制像素 |
       ↓
...
| 操作类型 | 触发情况 | 性能消耗 | 重排 (Reflow/Layout) |
| --- | --- | --- | --- |
| 改变元素几何信息（宽、高、位置） | 高 | 重绘 (Repaint) | 改变元素样式（颜色、背景） |
| 中 | 两者区别 | 重排包含重绘 | 重绘不改变布局 |
🔹 示例
// 重排
