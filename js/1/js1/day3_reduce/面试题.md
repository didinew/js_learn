...
它接收两个核心参数：1. 累加器（accumulator） —— 保存每一步计算的结果。
2. 当前元素（currentValue） —— 数组正在处理的元素。
...
| 用途 | for 循环写法 | reduce 写法 |
| --- | --- | --- | --- |
| 求和 | 累加 sum | arr.reduce((acc, val) => acc + val, 0) | 求最大值 |
| 遍历比大小 | arr.reduce((max, val) => val > max ? val : max, -Infinity) | 数组扁平化 | for 循环 push |
